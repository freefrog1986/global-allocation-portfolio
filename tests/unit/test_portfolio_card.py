...
class TestBreakdownBarChart:
    ...
    def test_y_axis_has_percent_formatter(self, journal: PortfolioJournal) -> None:
        """Y 轴 label 后面要带 % 后缀（第六轮反馈："外轴只有数字没有百分号"）。

        VChart 标准做法：axes[*].label.formatMethod = 'val => val + "%"'
        """
        spec = self._get_bar(journal)
        assert "axes" in spec
        # left 轴（Y 轴）的 label 配了 formatMethod
        left_axis = next(a for a in spec["axes"] if a["orient"] == "left")
        assert "label" in left_axis
        assert "formatMethod" in left_axis["label"]
        # formatMethod 是字符串（JS 函数表达式），不能是 dict
        assert isinstance(left_axis["label"]["formatMethod"], str)
        # 函数体里得有 "%" 字面量
        assert "%" in left_axis["label"]["formatMethod"]
